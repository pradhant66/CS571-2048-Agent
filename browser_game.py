# -*- coding: utf-8 -*-
"""
Browser game control for 2048.
Provides interfaces to control a 2048 game running in a browser.
"""
...
class Generic2048Control(object):
    """Base class for browser 2048 game control."""
    ...
    def setup(self):
        """Setup the browser control. Must be implemented by subclasses."""
...
class Fast2048Control(Generic2048Control):
    ...
    def get_board(self):
        ...
        Returns:
            4x4 list of lists, where values are log2 of tile values (0=empty, 1=2, 2=4, 3=8, etc.)
        """
    ...
    def execute_move(self, move):
        ...
        Args:
            move: 0=up, 1=down, 2=left, 3=right (UDLR ordering)
        """
...
class Keyboard2048Control(Generic2048Control):
    ...
    def setup(self):
        """Setup tile container reference."""
        self.execute('''
    ...
    def execute_move(self, move):
        """
        Execute a move using keyboard events.
        
        Args:
            move: 0=up, 1=down, 2=left, 3=right (UDLR ordering)
        """
        key = [38, 40, 37, 39][move]  # Arrow keys: up, down, left, right
...
def log2_to_value(log2_val):
    """Convert log2 representation to actual tile value."""
    if log2_val == 0:
        return 0
    return 2 ** log2_val


def value_to_log2(value):
    """Convert actual tile value to log2 representation."""
    if value == 0:
        return 0
    return int(round(math.log(value, 2)))


def convert_board_log2_to_values(board_log2):
    """Convert board from log2 representation to actual tile values."""
    return [[log2_to_value(cell) for cell in row] for row in board_log2]


def convert_board_values_to_log2(board_values):
    """Convert board from actual tile values to log2 representation."""
    return [[value_to_log2(cell) for cell in row] for row in board_values]
